import numpy as np
import sys
from read_focus_boundary import read_focus_boundary
...
class FourSurf(object):
    ...
    @classmethod
    def get_surface_points(cls,plasma_input,ntor=10,npol=10):
      """plasma_input: focus plasma file name
      return a (3, ntor*npol) array of points on the plasma boundary"""
      plasma = cls.read_focus_input(plasma_input)
      theta = np.linspace(0,2*np.pi,ntor)
      zeta = np.linspace(0,2*np.pi,npol)
      pts = np.array(plasma.xyz(theta,zeta))
      return pts
